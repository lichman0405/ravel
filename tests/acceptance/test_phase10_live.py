...
- **P10-17** asks whether the *whole thing* works when every seat is a live
  ...
  scripted seats would be a test of the scripts.
...
from ravel.domain.enums import AccessStatus, NodeType, ProjectStatus
...
#: An objective that needs both benches *at once*. Master decides the plan —
#: that is the point of a live Master — so this says what the user asked for and
#: nothing about how to stage it.
#:
#: It used to say "compute the candidate series first, then confirm the leading
#: candidate with a laboratory measurement", and that wording made the item
#: unable to test what it claims. Serialising the benches made the experiment
#: reachable only through a *passing* computation, and under V0's mock backends
#: a passing computation is not something a live Review will give: it reads
#: `kind: simulated`, `provenance: mock-compute:COMPUTE_SUCCESS` and the note
#: saying the output is not admissible as evidence, and it fails the node on
#: every criterion written against real work. That is the correct verdict and
#: the marking is the point of the mock backends. The consequence is what
#: matters here: no candidate is ever nominated, an experiment to confirm one
#: would be a measurement with nothing to measure, a live Master says so in
#: writing — "no EXPERIMENT node was ever committed to the DAG" — and the fifth
#: seat is never asked to do anything. The item then fails on a run whose plan
#: was *right*, which its own reasoning forbids: a plan that answers half the
#: objective is a worse plan, not a failed test.
#:
#: So the two benches are stated as two deliverables that do not wait on each
#: other. A project driven by this objective reaches both seats under mock
#: backends, and the run is decided by what the seats do rather than by which
#: of them the plan happened to need.
LIVE_OBJECTIVE = (
    "Establish whether niobium doping raises the conductivity of titanium dioxide "
    "by at least 15% over the undoped baseline. Two things are needed and neither "
    "waits on the other: the undoped TiO2 baseline conductivity measured in the "
    "laboratory, and the candidate Nb_xTi_(1-x)O2 series computed. I will put the "
    "two together myself."
)
...
def project(database: Database, clean: None) -> Project:
    """This module's project, stated as the question P10-17 is about.
...
async def test_p10_17_five_live_agents_drive_a_project_to_an_ending(
    ...
    This is the item Phase 10 exists for. The supervisor is pointed at a
    database and nothing else; it discovers the project, builds five seats from
    the pinned harness, and drives the loop until the project ends. Every turn
    in that loop is a real model call against a real tool server, and the
    work the plan calls for runs on the mock backends through Temporal.
    ...
    assert {node.node_type for node in nodes} >= {NodeType.COMPUTATION}, (
        "the objective asked for the series to be computed and the plan has no "
        f"computation in it: {sorted(node.node_type.value for node in nodes)}"
    )
